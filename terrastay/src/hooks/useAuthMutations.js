...
export const useLogin = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      const roleHome = role === 'ADMIN' ? '/admin' : role === 'MANAGER' ? '/manager/dashboard' : '/';
      const target = role === 'ADMIN' || role === 'MANAGER' ? roleHome : (from && from !== '/login' && from !== '/register' ? from : roleHome);
